# FSDP LoRA Fine Tuning Deployment Guide

We will be deploying a distributed LoRA fine tuning job using PyTorch FSDP (Fully Sharded Data Parallel) on OpenShift AI with the Kubeflow Training Operator.
...
| File | Purpose |
|------|---------|
| `lora-adapter-builder.py` | The training script. Loads the base model with QLoRA, formats data into chat templates, trains with SFTTrainer using FSDP sharding and logs to MLflow. |
...
```bash
oc delete configmap decoder-fsdp-train-script
oc create configmap decoder-fsdp-train-script --from-file=train.py=lora-adapter-builder.py
```
...
```bash
oc get trainjob decoder-fsdp-trainjob
```

Watch the pods:

```bash
oc get pods -l trainer.kubeflow.org/trainjob-name=decoder-fsdp-trainjob -w
```
...
With FSDP, both GPUs should show similar utilization since the model parameters and optimizer states are sharded evenly across GPUs. Memory usage per GPU will be lower compared to DDP because FSDP distributes the model state rather than replicating it.

Training progress and metrics (loss, eval_loss, learning rate) are also available in your MLflow tracking server under the experiment name `decoder-fsdp-finetuning`.
...
```bash
oc delete trainjob decoder-fsdp-trainjob
oc delete trainingruntime decoder-fsdp-runtime
oc delete configmap decoder-fsdp-train-script decoder-fsdp-train-args

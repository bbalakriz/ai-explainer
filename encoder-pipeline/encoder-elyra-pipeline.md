...
## Automated LoRA Fine-Tuning pipeline for decoder model 
...
- The following notebooks ready in your repository:
  - `encoder_lora_training.ipynb` (SFT fine-tuning)
  - `encoder_lora_evaluation.ipynb` (evaluation)
  - `encoder_lora_model_merge.ipynb` (model merge)
...
Open `encoder_lora_training.ipynb` and `encoder_lora_evaluation.ipynb`. Replace the `getpass()` block with a direct environment variable fetch so the pipeline can authenticate automatically:
...
```bash
git add encoder_lora_training.ipynb encoder_lora_evaluation.ipynb
